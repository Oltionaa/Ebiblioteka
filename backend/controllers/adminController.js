...
class AdminController {
  async getAllUsers(req, res) {
    try {
      ...
      console.log(err);
      res.status(500).json({
        message: "Gabim gjatë marrjes së përdoruesve.",
      });
    }
  }
  ...
  async addUser(req, res) {
    ...
    try {
      ...
      if (last.length > 0) {
        nextNumber =
          parseInt(last[0].numriKarteLexuesi.split("-")[1]) + 1;
      }
      ...
      await pool.query(
        `INSERT INTO perdoruesi 
         (emri, mbiemri, email, fjalekalimi, numriKarteLexuesi, roli)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [emri, mbiemri, email, hashedPassword, newCard, roli]
      );

      await addLog(
        1,
        `Shtoi përdoruesin ${emri} ${mbiemri}`,
        "Admin",
        req.ip
      );
      ...
      console.log(err);
      res.status(500).json({
        message: "Gabim gjatë shtimit të përdoruesit.",
      });
    }
  }
  ...
  async updateUser(req, res) {
    ...
    try {
      ...
      res.status(500).json({
        message: "Gabim gjatë modifikimit.",
      });
    }
  }
  ...
  async deleteUser(req, res) {
    ...
    try {
      await pool.query(
        "DELETE FROM perdoruesi WHERE id_perdoruesi=?",
        [id]
      );

      await addLog(
        1,
        `Fshiu përdoruesin ID ${id}`,
        "Admin",
        req.ip
      );
  ...
  async getMonthlyStats(req, res) {
    try {
      ...
      console.log(err);
      res.status(500).json({
        message: "Gabim në monthly stats.",
      });
    }
  }
}
...
export const getSystemStats = (req, res) =>
  controller.getSystemStats(req, res);

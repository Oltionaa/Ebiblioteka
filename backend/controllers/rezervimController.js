...
class RezervimController {
  async rezervoLiber(req, res) {
    ...
    try {
      ...
      res.json({
        message: `Rezervimi u bë me sukses! Kthimi deri më ${dataRezervuar}`,
      });
    } catch (err) {
      console.log(err);
  ...
  async getRezervimetByUser(req, res) {
  ...
  async fshiRezervim(req, res) {
    ...
    try {
      ...
      if (result.affectedRows === 0)
        return res
          .status(404)
          .json({ message: "Rezervimi nuk ekziston!" });
  ...
  async getAllRezervimet(req, res) {
    try {
      const [rows] = await pool.query(
      ...
      res
        .status(500)
        .json({ message: "Gabim gjatë marrjes së rezervimeve!" });
    }
  }

  async miratoRezervim(req, res) {
    const { id_rezervimi } = req.params;
    ...
    try {
      ...
      if (!rez)
        return res
          .status(404)
          .json({ message: "Rezervimi nuk u gjet!" });
...
const controller = new RezervimController();

export const rezervoLiber = (req, res) =>
  controller.rezervoLiber(req, res);

export const getRezervimetByUser = (req, res) =>
  controller.getRezervimetByUser(req, res);

export const fshiRezervim = (req, res) =>
  controller.fshiRezervim(req, res);

export const ndryshoRezervim = (req, res) =>
  controller.ndryshoRezervim(req, res);

export const getAllRezervimet = (req, res) =>
  controller.getAllRezervimet(req, res);

export const miratoRezervim = (req, res) =>
  controller.miratoRezervim(req, res);

export const refuzoRezervim = (req, res) =>
  controller.refuzoRezervim(req, res);
